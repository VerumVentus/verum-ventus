<template>
  ...
  <div class="grid grid-cols-2 gap-4">
    <v-plotly
      ...
      :key="`frequencyTweetTypeData`"
      :data="frequencyTweetTypeData"
...
const frequencyTweetTypeData = ref([]);
  ...
  Papa.parse(
    'https://raw.githubusercontent.com/VerumVentus/twitter-scraper/main/exploration/3_dropped_following_outliers.csv',
    {
      ...
      complete(results) {
        ...
        const tweetTypes = new Set(csvData.map((row) => row['Tweet Type']));
        tweetTypes.delete(undefined);
        ...
        sortedAccountTypes.forEach((accType, index) => {
          ...
          const typeFrequency = [...tweetTypes].map((tweetType) => {
            const subset = csvData.filter(
              (row) =>
                row['Tweet Type'] === tweetType &&
        ...
        frequencyTweetTypeData.value = data;

        const yearlyData = [];

        [...tweetTypes].forEach((type, index) => {
          if (!type) {
            return;
          }
          ...
          const yearlyTweetCounts = sortedYears.map((year) => {
            const subset = csvData.filter(
              (row) => row['Year'] === year && row['Tweet Type'] === type
            );
            return subset.length;
          });
